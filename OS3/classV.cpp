...
ProcessJobs::ProcessJobs(int StartTime, int PID, int ProbIORequest, int Length)
{
	StartTime = StartTime;
	PID = PID;
	ProbIORequest = ProbIORequest;
	Length = Length;
}



int ProcessJobs::RandomNumber(int start, int endtime)
{
	int RandomNumber;
...
int ProcessJobs::CPU(int quantum, queue<jobs> &ready, queue<jobs> &IO, int &Throughput, int &JobsInSystem, int CurrentSystemTime)
{
	int newIOProb;
	if(!ready.empty()){
		jobs incpu;
...
void ProcessJobs::IO_Process(int quantum, queue<jobs> &ready, queue<jobs> &IO, int &Throughput, int &JobsInSystem){
	if(IOTimeStart != 0 ){
	...
	if(!IO.empty() && EnterIO){
		...
		//cout << "IO IOLength1 is :" << IOLength << endl;
		IOLength = IOLength/1000;
		cout << "IO IOLength2 is :" << IOLength << endl;
		time_t CurrentTime = IOTimeStart;
		while(IOLength > (CurrentTime - IOTimeStart))
		{
			CurrentTime = time(NULL);
			if(IOLength <= (CurrentTime - IOTimeStart))
			{
...
int ProcessJobs::JobsStillInSystem(queue<jobs> ready, queue<jobs> IO)
{
	int JobsInSystem = 0;
	while(!ready.empty())
	{
...
/*
int ProcessJobs::TotalJobsSkip(queue<jobs> incoming)
{
	int JobsSkipped;
